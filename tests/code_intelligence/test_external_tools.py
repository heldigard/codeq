...
def test_ast_grep(fixture_dir: Path) -> None:
    ...
    assert "hello" in result.stdout or result.returncode == 0, f"ast-grep failed: {result.stdout}"
...
def test_ast_grep_java_expression(fixture_dir: Path) -> None:
    ...
    result = run(
        ["ast-grep", "run", "-p", "new Customer($$$)", "--lang", "java", str(fixture_dir)],
        check=False,
    )
...
def test_shellcheck() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "SC2086" in result.stdout or result.stderr, f"shellcheck did not flag SC2086: {result.stdout}"
