...
def test_codeq_typescript(fixture_dir: Path) -> None:
    ...
    assert "version-check.service.ts" in result.stdout, f"codeq find TS failed: {result.stdout}"
    ...
    assert "this.version" in result.stdout, f"codeq body TS method failed: {result.stdout}"
...
def test_codeq_typescript_after_generic_field(fixture_dir: Path) -> None:
    ...
    assert "submitMessage" in sig.stdout, f"codeq sig regex fallback failed: {sig.stdout}{sig.stderr}"
    ...
    for kw in ("if", "while", "for", "effect", "clearTimeout", "try"):
        ...
        bad = [
            line for line in outline.stdout.splitlines()
            if "method" in line and (f" {kw} " in line or line.endswith(f"method        {kw}"))
        ]
...
def test_codeq_refs_ts_filters_method_declaration(fixture_dir: Path) -> None:
    ...
    service = fixture_dir / "angular-component.service.ts"
    refs = run(["codeq", "refs", "onMessagesScroll", "-l", "typescript", "-p", str(fixture_dir)])
...
"""
    ...
    found = {line.split()[-1] for line in deps.stdout.splitlines() if line.strip()}
